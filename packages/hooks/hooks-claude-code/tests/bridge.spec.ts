...
describe('hooks-claude-code bridge — load resilience', () => {
  ...
  it('an invalid regex matcher is reported and registers no hooks', async () => {
    ...
    const agent = await ctx.agentLoop.create(SessionId('invalid-claude-matcher'), { provider: 'mock', model: 'mock' })
    agent.followup(createUserMessage({ content: [{ type: 'text', text: 'go' }], source: { kind: 'user' } }))
    await waitForIdle(ctx, agent)
    expect(adapter.requests).toHaveLength(1)
  ...
  it('an invalid matcher on an unsupported event does not disable supported hooks', async () => {
    ...
    const warn = vi.fn()
    const ctx = await harness(dir, adapter, (ctx) => { ctx.logger.warn = warn as never })
    const agent = await ctx.agentLoop.create(SessionId('unsupported-claude-matcher'), { provider: 'mock', model: 'mock' })
    agent.followup(createUserMessage({ content: [{ type: 'text', text: 'go' }], source: { kind: 'user' } }))
    await waitForIdle(ctx, agent)
  ...
  it('a SessionEnd hook fires when the agent is disposed (agent/disposed → SessionEnd)', async () => {
    // The bridge routes `agent/disposed` (emitted once when an agent's owning fiber
    // is torn down) to the detached SessionEnd hook. Create + announce an agent,
    // then release it so it leaves the registry; the SessionEnd hook must run.
    ...
    // `dispose()` exists on the runtime service (disposes every live agent →
    // `agent/disposed`) but isn't on the published type surface; a narrow cast keeps
    // this compile-safe while still exercising the real teardown path.
    await (ctx.agentLoop as unknown as { dispose(): Promise<void> }).dispose()
    await waitFor(() => existsSync(marker))
